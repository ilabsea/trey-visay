...
export default class PrivateSchoolScreen extends Component {
  ...
  componentWillMount() {
    this.privateSchools = schoolList.filter(school => school.category == 'សាលាឯកជន');

    this.state = {
      schools: this.privateSchools,
      location: '',
      major: '',
      majors: [],
      provinces: [...new Set(this.privateSchools.map(school => school.province))],
    }
  }
  ...
  _onChangeProvince(province) {
    ...
    let schools = this.privateSchools;
    let majors = [];

    if (!!province) {
      schools = this.privateSchools.filter(school => school.province == province);
